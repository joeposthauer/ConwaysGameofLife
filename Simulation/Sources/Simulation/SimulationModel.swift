...
public let simulationReducer = Reducer<SimulationState, SimulationState.Action, SimulationEnvironment>.combine(
    ...
    Reducer<SimulationState, SimulationState.Action, SimulationEnvironment>{ state, action, env in
        ...
        case .animate:
            state.start = true
            //state.gridState.stop = false
            state.gridState.start = true
            return Just(SimulationState.Action.stopanimate)
                .delay(for: 0.1, scheduler: DispatchQueue.main)
                .eraseToEffect()
        case .stopanimate:
            state.gridState.start = false
            state.start = false
            //state.gridState.stop = true
            return .none
